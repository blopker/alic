...
#[cfg(test)]
mod tests {
    /// Guard against src/bindings.ts drifting from the Rust commands and
    /// events. The file is normally only regenerated by a debug-build app
    /// launch; this test exports fresh bindings and compares. On failure,
    /// copy src-tauri/target/bindings-check.ts over src/bindings.ts.
    #[test]
    fn bindings_are_up_to_date() {
        let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let out = manifest_dir.join("target/bindings-check.ts");
        super::specta_builder()
            .export(specta_typescript::Typescript::default(), &out)
            .expect("Failed to export bindings");
        let generated = std::fs::read_to_string(&out).expect("Failed to read exported bindings");
        let committed = std::fs::read_to_string(manifest_dir.join("../src/bindings.ts"))
            .expect("Failed to read src/bindings.ts");
        assert_eq!(
            generated, committed,
            "src/bindings.ts is out of date. Copy src-tauri/target/bindings-check.ts over src/bindings.ts."
        );
    }
}
