...
async processImg(parameters: ProfileData, file: FileEntry, parallelImages: number) : Promise<Result<CompressResult, AlicError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("process_img", { parameters, file, parallelImages }) };
} catch (e) {
...
export type SettingsData = { version: number; theme: ThemeKind; threads: number; default_profile_id: number | null; profiles: ProfileData[] }
